...
namespace HRMS.Controller
{
    ...
    public class AttendanceService
    {
        ...
        // Resolve both numeric id and employee code if available (from JSON body, token claims, or DB lookup)
        private static async Task<(int? employeeId, string? employeeCode)> ResolveEmployeeIdentity(HttpContext ctx, JsonDocument? json, HrmsContext db)
        {
            int? employeeId = null;
            ...
            if (json != null)
            {
                ...
                JsonHelpers.TryGetInt(root, new[] { "EmployeeID", "EmployeeId", "employeeId", "id", "empId", "emp_id" }, out employeeId);
                JsonHelpers.TryGetString(root, new[] { "EmployeeCode", "employeeCode", "employee", "Employee", "employee_code" }, out employeeCode);
            }

            // Try token claims if not present
            if (!employeeId.HasValue || string.IsNullOrWhiteSpace(employeeCode))
            {
                var user = ctx.User;
            ...
            // If code is known but numeric id missing, lookup
            ...
            // If id present but code missing, lookup code
            if (employeeId.HasValue && string.IsNullOrWhiteSpace(employeeCode))
            {
                var emp = await db.EmployeeBasics
                    ...
                    .FirstOrDefaultAsync();
                if (emp != null && !string.IsNullOrWhiteSpace(emp.EmployeeCode))
                {
                    employeeCode = emp.EmployeeCode;
                }
            }
            ...
            return (employeeId, string.IsNullOrWhiteSpace(employeeCode) ? null : employeeCode);
        }
        ...
        public async Task Insert(HttpContext httpContext, HrmsContext db)
        {
            ...
            try
            {
                ...
                var identity = await ResolveEmployeeIdentity(httpContext, json, db);
                var employeeId = identity.employeeId;
                var employeeCode = identity.employeeCode;
                ...
                if (action.Equals("checkin", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // update existing - ensure EmployeeCode is present
                    if (string.IsNullOrWhiteSpace(existing.EmployeeCode) && !string.IsNullOrWhiteSpace(employeeCode))
                ...
                else
                {
                    ...
                    else a = new Attendance { EmployeeId = employeeId.Value, EmployeeCode = employeeCode, CreatedAt = DateTime.UtcNow };
                    ...
                    if (json != null)
                    {
                        ...
                        {
                            if (s is not null) a.Status = s;
                        }

                        if (JsonHelpers.TryGetTime(root, new[] { "CheckInTime", "checkInTime" }, out var cit) && cit.HasValue)
                            a.CheckInTime = cit.Value;

                        if (JsonHelpers.TryGetTime(root, new[] { "CheckOutTime", "checkOutTime" }, out var cot) && cot.HasValue)
                            a.CheckOutTime = cot.Value;

                        // allow explicit employee code override in payload
                        if (JsonHelpers.TryGetString(root, new[] { "EmployeeCode", "employeeCode", "employee_code" }, out var ec))
                            a.EmployeeCode = ec; // may be null => allowed
                    }

                    // if employee code still missing, set it (especially for new rows)
                    if (string.IsNullOrWhiteSpace(a.EmployeeCode) && !string.IsNullOrWhiteSpace(employeeCode))
   ...
   public class LeaveRequestsService
{
    ...
    public async Task Insert(HttpContext httpContext, HrmsContext db)
    {
        ...
        try
        {
            // Read raw body and log it for debugging (helpful for Postman/frontend checks)
            var body = await new StreamReader(httpContext.Request.Body).ReadToEndAsync();
            Console.WriteLine("[LeaveRequests/Insert] RAW BODY: " + (body ?? "<empty>"));
            JsonDocument? json = string.IsNullOrWhiteSpace(body) ? null : JsonDocument.Parse(body);
            ...
            // token fallback (if request omitted employee identity)
            if (!employeeId.HasValue)
            {
            ...
            if (json != null)
            {
                ...
                if (JsonHelpers.TryGetString(root, new[] { "LeaveType", "leaveType", "Type", "type", "leave_type" }, out var lt) && lt != null)
                    lr.LeaveType = lt;

                // Dates - accept many key names and formats
                if (JsonHelpers.TryGetDate(root, new[] { "FromDate", "fromDate", "from", "from_date", "startDate", "start" }, out var fromDate) && fromDate.HasValue)
                    lr.FromDate = fromDate.Value;

                if (JsonHelpers.TryGetDate(root, new[] { "ToDate", "toDate", "to", "to_date", "endDate", "end" }, out var toDate) && toDate.HasValue)
                ...
                if (root.TryGetProperty("Reason", out var rr) || root.TryGetProperty("reason", out rr) || root.TryGetProperty("reasonText", out rr) || root.TryGetProperty("reason_text", out rr))
                {
                ...
                // Optional: accept explicit Days from payload (we still recalc below)
                if (JsonHelpers.TryGetInt(root, new[] { "Days", "days" }, out var daysFromPayload) && daysFromPayload.HasValue)
                {
                    lr.Days = (decimal)daysFromPayload.Value;
                }
            }

            // fill employee code if still missing
            ...
            // validation: require at least 1 day
            if (lr.Days <= 0)
            {
                resp.Result = false;
                resp.Message = "Invalid FromDate/ToDate range. Ensure ToDate is same or after FromDate.";
                resp.Data = new { fromDate = lr.FromDate.ToString(), toDate = lr.ToDate.ToString(), days = lr.Days };
                httpContext.Response.StatusCode = 400;
                await httpContext.Response.WriteAsJsonAsync(resp);
                return;
            }
    ...
    public async Task GetAll(HttpContext httpContext, HrmsContext db)
    {
        ...
        try
        {
            ...
            if (json != null)
            {
                ...
                JsonHelpers.TryGetDate(root, new[] { "FromDate", "fromDate", "from", "startDate", "start" }, out from);
                JsonHelpers.TryGetDate(root, new[] { "ToDate", "toDate", "to", "endDate", "end" }, out to);
            }
